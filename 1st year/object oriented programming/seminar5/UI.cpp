...
{   cout<<"Enter old artist: ";
    ...
    try
    {
        this->serv.updateSongFromRepository(artist, title, minutes, seconds, link, old, tit);
    }
    catch (SongException& e)
    {
        for (auto s : e.getErrors())
            ...
            while (true)
            {
                ...
                switch (commandRepo)
                {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        this->updateSongToRepo();
                    case 5:
                        try {
                            this->serv.undo();
                        }
                        catch(...) {
                            std::cout << "No undo to be done!\n";
                        }
                        break;
                    case 6:
                        try {
